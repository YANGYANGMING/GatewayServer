...
from GWS.permission_list import perm_dic
from django.conf import settings


def perm_check(*args, **kwargs):
    """
    判断权限，通过检查收到的URL的name、method、args、kwargs、自定义钩子函数
    :param args:
    :param kwargs:
    :return:
    """
        ...
        if per_url_name == current_url_name:
            ...
            if per_method == request.method:
                ...
                for item in perm_args:
                    ...
                    if not request_method_func.get(item, None):  # request字典中无此参数
                        print("arg not match......")
                        args_matched = False
                ...
                for k, v in perm_kwargs.items():
                    ...
                    # 匹配上了特定的参数及对应的参数值，比如，需要request对象里必须有一个叫user_id=3的参数
                    if not arg_val == str(v):
